...
class studentdata(APIView):
    ...
    def get(self, request, formate=None, id=None):
        ...
        return Response({'response': 'this is only of student user'})

    def post(self, request):
        if User.objects.filter(
                username=request.user, groups__name='student').exists():
            return Response({'response': 'Student user is not authorized for add data'})
        stu = request.data
        serializer = useSerializer(data=stu)
        if serializer.is_valid():
            serializer.save()
            return Response({'response': 'data saved'})
        return Response({'response': 'data cant saved'})
